...
async function generateAlbumPairReasoning(
  ...
  const systemPrompt = `Write one natural sentence (≤20 words) explaining what makes this album pairing interesting for discovering music taste. No names, no generic phrases.
...
B: ${a2}`
  ...
  try {
    const response = await anthropic.messages.create({
      model: 'claude-haiku-4-5-20251001',
      system: systemPrompt,
      messages: [{ role: 'user', content: 'Explain this pairing.' }],
      temperature: 0.7,
      max_tokens: 60
    })
    ...
    const vibe1     = album1.personal_vibes[0] ?? ''
    const vibe2     = album2.personal_vibes[0] ?? ''

    return yearDiff > 10
      ? `This pairing contrasts ${vibe1 ? vibe1 + ' ' : ''}${genres1} with ${vibe2 ? vibe2 + ' ' : ''}${genres2}, spanning ${yearDiff} years of musical evolution.`
      : `This pairing explores the differences between ${vibe1 ? vibe1 + ' ' : ''}${genres1} and ${vibe2 ? vibe2 + ' ' : ''}${genres2} approaches.`
  }
}
